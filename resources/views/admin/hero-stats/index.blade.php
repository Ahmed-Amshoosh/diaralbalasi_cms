...
@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <button onclick="openAddModal()" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 transition-all shadow-sm hover:shadow-md">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="openEditModal({{ $stat->id }}, '{{ $stat->number }}', '{{ addslashes($stat->getTranslation('label', 'ar')) }}', '{{ addslashes($stat->getTranslation('label', 'en')) }}', {{ $stat->order }}, {{ $stat->is_active ? 1 : 0 }})"
                                ...
                                <button onclick="confirmDelete('{{ route('admin.hero-stats.destroy', $stat->id) }}', '{{ addslashes($stat->getTranslation('label', app()->getLocale())) }}')"
    ...
    <div id="statModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="relative inline-block w-full max-w-2xl px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-xl shadow-xl sm:my-8 sm:align-middle sm:p-6">
                <div class="flex justify-between items-center mb-5 border-b pb-3">
                    <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                        <i class="fas fa-chart-bar text-blue-500"></i> <span id="modalTitleText">{{ __('messages.add_stat') }}</span>
                    </h3>
                ...
                <form id="statForm" method="POST" novalidate>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 text-start">
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.stat_number') }} <span class="text-red-500">*</span></label>
                            <input type="text" name="number" id="number" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"  placeholder="{{ __('messages.stat_number_placeholder') }}" required>
                            <p class="field-error text-red-500 text-xs mt-1.5 hidden" id="error-number"><i class="fas fa-exclamation-circle"></i> {{ __('messages.this_field_is_required') }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.stat_label') }} (عربي) <span class="text-red-500">*</span></label>
                            <input type="text" name="label_ar" id="label_ar" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"  required>
                        ...
                        <div class="relative">
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.stat_label') }} (English) <span class="text-red-500">*</span></label>
                            <input type="text" name="label_en" id="label_en" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"  required>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100 text-start" >
                        <div>
                            ...
                            <input type="number" name="order" id="order" value="0" min="0" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                        </div>
            ...
            {{ __('messages.hero_is_active') }}
                        ...
                        <button type="submit" class="mt-2 px-8 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold shadow-md hover:shadow-lg transition-all flex items-center gap-2">
