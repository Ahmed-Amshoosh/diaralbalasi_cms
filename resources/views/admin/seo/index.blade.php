    ...
    <div class="space-y-6">
        <form action="{{ route('admin.seo.update') }}" method="POST" enctype="multipart/form-data">
                ...
                {{ __('messages.seo_chars_count') }}
                        ...
                        <div>
                            ...
                            <textarea name="description_ar" id="description_ar" maxlength="160" rows="3"
                                      ...
                                      dir="rtl" >{{ old('description_ar', $seo['description'] ?? '') }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="text" name="keywords_ar"
                                   value="{{ old('keywords_ar', $seo['keywords'] ?? '') }}"
                                   class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                                   dir="rtl" placeholder="كلمة1, كلمة2, كلمة3">
                ...
                {{ __('messages.seo_chars_count') }}
                        ...
                        <div>
                            ...
                            <textarea name="description_en" id="description_en" maxlength="160" rows="3"
                                      class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none"
                                      dir="ltr" >{{ old('description_en', $seo['description'] ?? '') }}</textarea>
                            <p class="text-xs text-gray-500 mt-1"><i class="fas fa-info-circle"></i> {{ __('messages.seo_description_hint') }}</p>
                        ...
                        <div>
                            ...
                            <input type="text" name="keywords_en"
                                   value="{{ old('keywords_en', $seo['keywords'] ?? '') }}"
                                   class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                                   dir="ltr" placeholder="keyword1, keyword2, keyword3">
                            <p class="text-xs text-gray-500 mt-1"><i class="fas fa-info-circle"></i> {{ __('messages.seo_keywords_hint') }}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="bg-gray-50 px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                        <i class="fas fa-share-alt text-purple-500"></i> {{ __('messages.seo_social') }}
                    </h3>
                </div>
                <div class="p-6 space-y-5">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.seo_og_image') }}</label>
                        @if($seo['og_image'])
                            <div class="mb-3">
                                <img src="{{ asset('storage/' . $seo['og_image']) }}" alt="OG Image" class="h-40 w-auto object-contain rounded-lg border border-gray-200 p-1">
                            </div>
                        @endif
                        <input type="file" name="og_image" accept="image/*" class="w-full text-sm text-gray-500 file:mr-3 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 cursor-pointer">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <select name="twitter_card" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                                <option value="summary" {{ ($seo['twitter_card'] ?? '') === 'summary' ? 'selected' : '' }}>Summary (صغيرة)</option>
                                <option value="summary_large_image" {{ ($seo['twitter_card'] ?? '') === 'summary_large_image' ? 'selected' : '' }}>Summary Large Image (كبيرة)</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.seo_twitter_site') }}</label>
                            <input type="text" name="twitter_site" value="{{ old('twitter_site', $seo['twitter_site'] ?? '') }}" placeholder="@username" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" dir="ltr">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="bg-gray-50 px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                        <i class="fas fa-cogs text-orange-500"></i> {{ __('messages.seo_advanced') }}
                    </h3>
                </div>
                <div class="p-6 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.seo_author') }}</label>
                            <input type="text" name="author" value="{{ old('author', $seo['author'] ?? '') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.seo_robots') }}</label>
                            <input type="text" name="robots" value="{{ old('robots', $seo['robots'] ?? 'index, follow') }}" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" dir="ltr">
                            <p class="text-xs text-gray-500 mt-1"><i class="fas fa-info-circle"></i> {{ __('messages.seo_robots_hint') }}</p>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.seo_canonical_url') }}</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-4 border-t border-gray-100">
                        <div>
                            ...
                            <input type="text" name="google_analytics" value="{{ old('google_analytics', $seo['google_analytics'] ?? '') }}" placeholder="G-XXXXXXXXXX" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" dir="ltr">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('messages.seo_google_tag_manager') }}</label>
                            <input type="text" name="google_tag_manager" value="{{ old('google_tag_manager', $seo['google_tag_manager'] ?? '') }}" placeholder="GTM-XXXXXXX" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" dir="ltr">
                        </div>
                    </div>
                    <button type="submit" class="px-8 mb-2 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold shadow-md hover:shadow-lg transition-all flex items-center gap-2">
